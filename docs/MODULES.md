...
## Module: main (`ci/landlock/kernelprobe`)
...
a launch can refuse a writable grant the profile would break. Its name
carries a digest of the policy it was generated as, and a launch stacks only
the name its installed source implies: a source edited or regenerated but
not reloaded names a profile that is not loaded, and the launch is refused
...
#### `func NameFor(roots []string) string`
NameFor returns the name of the profile Profile generates for roots, which
...
rendered with the bare stem as its name: two sources share a name exactly
when they are the same policy.
...
#### `func StackOnExec(profile string) error`
...
#### `func Parse(src []byte) (*Layer, error)`
Parse reads an installed profile source: the roots its header records, and
the name of the profile it describes. The source must be exactly what
Profile generates for those roots — an edited source, or one from a
...
## Module: containment (`pkg/containment`)
