...
Offline drift sentry: compares the upstream versions pinned in pkg/versions against the npm registry's latest published release for each declared harness package. Prints a Markdown table or JSON and exits 0 when every pin matches, 1 on drift, and 2 when the pins could not be read or the registry could not be reached.
...
A scriptable stand-in for an interactive coding harness, used by the PTY-driven integration tests. It reads a JSON script named by an environment variable, switches its PTY slave to raw mode like a real TUI, and replays the script's timeline of painted frames, input waits and exits. Not a product binary.
...
HTTP + Server-Sent-Events gateway that exposes pkg/chat so non-Go clients can drive multi-turn harness conversations across a process boundary. It serves conversation, control-token, message, input, history and screen routes under /v1 and streams turn and input events as SSE frames. v1 ships no authentication and is meant to be bound to localhost.
...
The CLI front-end over pkg/wrapper and pkg/chat. It offers transparent PTY passthrough, a one-shot run that reads a prompt from stdin and prints the reply, a machine-readable structured-run that emits one turnproto JSON result line, and tmux-backed detached sessions with attach, status, kill and list subcommands. The dangerous sandbox-defaults injection is deliberately owned here, at the CLI boundary.
...
The Daytona provisioner: elastically provisioned remote sandboxes exposed through the internal/env Workspace contract. It depends only on SDK interfaces declared locally rather than on the real Daytona SDK, so unit tests inject a fake and the live client is wired in only for the live test tier.
...
The script format, fluent builder and one-time binary build behind the scriptable fake harness. It gives the chat tests a real program on a real PTY, so replays exercise the actual screen emulator, turn watcher and idle-completion timing that corpus replay alone cannot reach.
...
The terminal-emulator bake-off command. It replays each recorded scenario's raw PTY bytes through every registered emulator candidate and reports fidelity against the expected snapshot, stability after settling, and throughput — the measurement behind the project's vt100 emulator choice.
...
Records a live harness session under wrapper supervision, capturing the raw PTY byte stream and scenario metadata into a corpus directory. It runs interactively or unattended from a JSON script of wait, send, sleep and interrupt steps, bounded by an idle timeout and a maximum session duration.
...
The Go chat-conversation API. Each Conversation owns one PTY-supervised harness process and exposes a FIFO control token, one assistant turn per Send, an event stream, request/answer handling for blocking prompts, transcript-backed history, and live permission-mode switching. Persistence is pluggable through the Store interface, and transports deliberately live outside this package.
...
Answers whether each harness declared in the version pins is installed on PATH and at what version, probing the binary's own version flag behind a cache keyed on binary path and modification time. It is read-only with respect to the system — it never installs, writes, or alters PATH — and treats 'not installed' and 'probe failed' as ordinary results rather than errors.
...
A curated static registry of the model identifiers each harness accepts, plus a pure parser for the interactive model-picker screen. It is deliberately offline, because no supported harness exposes a machine-readable model list, so callers can validate or offer models without launching a CLI.
...
The public host-side client for running one structured turn inside a workspace. It uploads the prompt as a file so argv can never carry it, executes the structured-run runner in the guest, parses the last JSON line of stdout into a structured result, and optionally downloads the transcript. A non-zero guest exit is returned as a parsed result rather than an error.
...
The per-harness orchestration and capability layer. Profiles resolve, once per run, which capabilities a harness actually offers — session-id extraction, resume arguments, stream parsing, hooks — while Run supervises a headless run and acquires its transcript through hooks or stream parsing, and RunTurn drives exactly one interactive turn on top of the chat layer. It is distinct from the TUI turn adapters under pkg/turns/harness.
...
The harness profile for the Claude Code CLI. It extracts the session id from Claude's machine-readable init event and supplies the resume argument prefix, with later capabilities covering stream parsing and hook installation. Importing the package registers the profile.
...
The in-process library for running exactly one headless harness turn and classifying it into the frozen structured-turn status vocabulary. It owns the unattended policy — quit after the turn, auto-answer blocking prompts affirmatively, prefer the harness's own transcript for the reply — and returns every classified outcome with a nil error, reserving errors for unclassifiable failures.
...
The read-only view of harness-owned session logs, together with the canonical harness-agnostic event model, its public and durable serializations, normalized token accounting, and the shared JSONL line helpers. It is a leaf package that imports neither the turns nor the chat layer, so anything may parse a harness log.
...
#### `UsageReader`
An optional capability a transcript reader may also implement: best-effort token accounting for one session, returning nothing when the transcript carried no usage.
...
Reads Claude Code's own JSONL session logs, locating a session by encoding the working directory into its projects path, and parses them into canonical transcript events plus optional token usage.
...
The neutral wire contract for one supervised turn: the status vocabulary, the canonical status-to-exit-code table, the frozen stderr deadline anchor, the structured result object, and the tolerant last-JSON-line parser hosts use to read it. Every exported name and JSON tag here is a frozen cross-repository contract.
...
Defines the per-harness adapter contract that turns screen snapshots and wrapper status into a small set of turn events, the optional capability interfaces an adapter may opt into, the model for interactive input requests, and the watcher that pumps a wrapper session and a screen into one event stream.
...
Reports the harness's permission posture as read off the rendered screen; a negative result means the screen carries no readable signal, never that a particular mode is absent. Implementations do not all report the same axis.
...
The Claude Code turn adapter. It detects end-of-turn from the thinking-summary line, reports busy from the interrupt footer and spinner, isolates the assistant reply from TUI chrome, parses trust and bypass-acceptance dialogs into structured input requests, reads the permission-mode footer, and supplies graceful quit, resume and transcript capabilities.
...
The Codex turn adapter. It completes a turn on a fresh end-of-turn footer, extracts or locates the resume session id, surfaces auto-dismissable startup interstitials and strictly-recognised approval dialogs as input requests, and reports Codex's collaboration-axis mode from the rendered screen.
...
The foundation layer: it launches a harness under a pseudoterminal, supervises the process, and classifies the run into a small normalized status vocabulary with a canonical error taxonomy. It also owns the execution-mode knobs — reasoning effort, model and permission rung — translating each into that harness's own launch arguments, and exposes a live session handle for attaching output, writing input and resizing.
...
Reflects Go DTO structs into the language-neutral field vocabulary that the cross-language conformance corpus freezes. It is the single place the Go-to-neutral mapping lives, shared by both generators so they emit identical field descriptions, and it deliberately hides Go-only distinctions a TypeScript consumer cannot observe.
...
A non-PTY mock harness binary used by the wrapper's own tests. Its flags select a canned behaviour — completed, failed, stuck, waiting for input, cost-limited, API-error and similar — so supervision and classification paths can be exercised deterministically.
