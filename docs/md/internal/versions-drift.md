...
> Both files carry claude-code `2.1.282` as of 2026-09-24, verified live against the installed
> 2.1.282 binary by `pkg/harness`'s `TestRunTurn_RealClaude*` and `pkg/chat`'s `TestTrustDialogLive`,
...
> `TestInterruptLive` and `TestStreamAccountLive` on 2.1.282; its replay corpus
...
> shape 2.1.270 already painted, so nothing was re-baked; 2.1.282 passed every live test unchanged,
> so nothing was re-baked for it either. Recordings are frozen renderings
...
> would drag this repo's pin *backwards* from 2.1.282 to 2.1.218.
