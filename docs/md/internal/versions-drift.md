...
> are recorded at 2.1.270, one release behind the pin, so the `interruptMarker` and tool-call surfaces
> are verified at 2.1.270 by replay and at 2.1.278 only insofar as the live tests touch them; the
> permission-mode footers remain anchored at 2.1.217. Recordings are frozen renderings
