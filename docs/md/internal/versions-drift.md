...
> Both files carry claude-code `2.1.281` as of 2026-09-23, verified live against the installed
> 2.1.281 binary by `pkg/harness`'s `TestRunTurn_RealClaude*` and `pkg/chat`'s `TestTrustDialogLive`:
...
> footers remain anchored at 2.1.217 (re-confirmed by hand on 2.1.281). All five live tests passed at
> 2.1.281 once `ComposerText` learned to read the empty composer's `Try "…"` placeholder as empty — a
> shape 2.1.270 already painted, so nothing was re-baked. Recordings are frozen renderings
...
> would drag this repo's pin *backwards* from 2.1.281 to 2.1.218.
