...
## HARNESS-WRAPPER-115 — dead-spawner false positive: the agent had *finished*, not died
...
**Why it landed here:** unchanged from §HARNESS-WRAPPER-23, §HARNESS-WRAPPER-26,
§HARNESS-WRAPPER-99, §HARNESS-WRAPPER-111 and §HARNESS-WRAPPER-112 — the observer
files a `dead-spawner` against the *task* it is watching (`HARNESS-WRAPPER-113`, in
this repo's fleet-db workspace), so the ticket lands in this workspace even though
every line of the defect lives in `orche`. This is the **sixth** recurrence of the
class.
...
**ORCHE-130 follow-up carrying Patches A + B + C** remains the outstanding action and no
