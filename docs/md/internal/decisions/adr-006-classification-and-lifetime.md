...
messages. There, silence is the resting state: Claude Code paints nothing while it sits at its
composer, nor while a turn waits on a background command. `chat.Open` built its `wrapper.Config` with
