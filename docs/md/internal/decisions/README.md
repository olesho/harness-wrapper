...
| 6 · Evolve public contracts deliberately | 002, 003, 005, 007, 008 | SSE frames gained a `type` field additively; no Go surface is published before a consumer can shape it; a policy without the AppArmor layer serializes, and fingerprints, exactly as before; the interrupt route and wire state came with their conformance fixtures; `OnEvent` was added beside `Events()`, whose contract stayed |
