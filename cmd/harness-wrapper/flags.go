...
func harnessWrapperFlagSet(a *harnessWrapperArgs) *flag.FlagSet {
	...
	fs.StringVar(&a.Contain.Kind, "contain", "", "run the harness inside an extra Landlock boundary (Linux, Landlock ABI 9+, or 6+ with the AppArmor socket layer installed): landlock; unrelated to --sandbox-defaults, which removes guardrails instead of adding one")
	...
	fs.IntVar(&a.Contain.MinABI, "contain-min-abi", 0, "with --contain: the lowest Landlock ABI to accept (6-10; default: detect — Landlock alone on ABI 9+, the AppArmor socket layer below it when installed); 9 refuses the socket layer")
