...
func harnessWrapperFlagSet(a *harnessWrapperArgs) *flag.FlagSet {
	...
	fs.StringVar(&a.Contain.Kind, "contain", "", "run the harness inside an extra Landlock boundary (Linux, ABI 9+, or 6+ with --contain-min-abi and the AppArmor socket layer): landlock; unrelated to --sandbox-defaults, which removes guardrails instead of adding one")
	...
	fs.IntVar(&a.Contain.MinABI, "contain-min-abi", 0, "with --contain: the lowest Landlock ABI to accept (default 9, minimum 6); below 9 pathname UNIX sockets are denied by the AppArmor socket layer (see contain-apparmor-profile)")
