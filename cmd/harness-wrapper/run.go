...
func runOneShot(args []string) int {
	...
	// the credential exemption in pkg/harnessenv): when harness-wrapper
...
// The Claude Code session-nesting env policy lives in pkg/harnessenv — the
// predicate, the CLAUDE_CODE_OAUTH_TOKEN exemption and the reasons for both.
// It was promoted out of this file so every library entrypoint (pkg/harness,
// pkg/oneshot and their live tests) can reuse the one implementation instead of
// mirroring it. (PUPPET-671)
//
// These two names stay as delegates: they are the spelling the rest of cmd/
// already calls.
func filterNestingEnv(src []string) []string { return harnessenv.Filter(src) }
...
// CLAUDE_CODE_OAUTH_TOKEN is deliberately preserved — see pkg/harnessenv.
func cleanedEnv() []string { return harnessenv.Cleaned() }
