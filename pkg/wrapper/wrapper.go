...
type Config struct {
	...
	// wrapper terminates a run (context cancellation, Stop, or a terminal
	// classification). Defaults to 5s.
	//
	// Both signals are delivered to the harness's process group, so tool
	// subprocesses are reaped with it rather than surviving the run — the
	// SIGKILL reaches any member still running once the grace period has
	// passed, even when the harness itself exited on the SIGTERM. See the
	// package doc for why that is safe, and Session.Wait for what it
	// promises.
