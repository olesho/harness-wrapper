...
// OpenCode is provider-agnostic, so there is no single anchored API-error
// format to match. APIError must stay nil until a corpus reveals one;
// SessionLimit likewise has no OpenCode-specific banner yet.
func TestPatternsHasNoAnchoredMatchers(t *testing.T) {
	if Patterns.APIError != nil {
		t.Error("APIError should be nil for opencode until a corpus pins a per-provider format")
	}
	if Patterns.SessionLimit != nil {
		t.Error("SessionLimit should be nil for opencode (no harness-specific banner observed)")
	}
}
