...
func (c *Conversation) Send(ctx context.Context, text string) (turnID string, err error) {
	...
	// The prompt and the submit key go out as SEPARATE writes, and the submit
	// is confirmed rather than assumed — see submit.go for the paste-collapse
	// failure that a single combined write loses to under load.
